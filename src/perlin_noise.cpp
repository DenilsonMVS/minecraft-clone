...
float PerlinNoise::noise(const glm::vec1 &pos) const {
	return this->noise(pos.x);
}

float PerlinNoise::noise(const glm::vec2 &pos) const {
	return this->noise(pos.x, pos.y);
}

float PerlinNoise::noise(const glm::vec3 &pos) const {
	return this->noise(pos.x, pos.y, pos.z);
}

float PerlinNoise::fractal(size_t octaves, const glm::vec1 &pos) const {
	return this->fractal(octaves, pos.x);
}

float PerlinNoise::fractal(size_t octaves, const glm::vec2 &pos) const {
	return this->fractal(octaves, pos.x, pos.y);
}

float PerlinNoise::fractal(size_t octaves, const glm::vec3 &pos) const {
	return this->fractal(octaves, pos.x, pos.z);
}

// Bellow code based on https://github.com/SRombauts/SimplexNoise
 ...
 */
inline int32_t PerlinNoise::fastfloor(float fp) {
 ...
 */
inline uint8_t PerlinNoise::hash(int32_t i) const {
 ...
 */
float PerlinNoise::grad(int32_t hash, float x) {
 ...
 * @return Noise value in the range[-1; 1], value of 0 on all integer coordinates.
 */
float PerlinNoise::fractal(size_t octaves, float x) const {
	float output    = 0.f;
    float denom     = 0.f;
 ...
 * @return Noise value in the range[-1; 1], value of 0 on all integer coordinates.
 */
float PerlinNoise::fractal(size_t octaves, float x, float y) const {
	float output = 0.f;
    float denom  = 0.f;
    float frequency = this->frequency;
    float amplitude = this->amplitude;
 ...
 * @return Noise value in the range[-1; 1], value of 0 on all integer coordinates.
 */
float PerlinNoise::fractal(size_t octaves, float x, float y, float z) const {
	float output = 0.f;
    float denom  = 0.f;
    float frequency = this->frequency;
    float amplitude = this->amplitude;
